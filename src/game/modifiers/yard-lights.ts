...
type Bolt = {
  ...
  /** Awaken matrix: stopped, no collide, falling to floor. */
  falling: boolean;
  /** Facing locked when enter falling (draw while vy≈0). */
  faceAng: number;
};
...
const ELITE_SPD = 920;
const KNOCK = 1180;
/** Near-ball radius (× ball.r) to trigger awaken freeze/fall. */
const AWAKEN_NEAR = 3.6;
/** Gravity while bolts drop after awaken freeze. */
...
/** Drawn length of the bolt sprite (art is 128×32). */
const BOLT_W = 28;
const ELITE_W = 36;
const TRAIL_LEN = 16;
...
function drawBoltTrail(ctx: CanvasRenderingContext2D, b: Bolt) {
  const pts = b.trail;
  ...
  for (let i = 1; i < pts.length; i++) {
    ...
    const a = pts[i - 1]!;
    const c = pts[i]!;
    ctx.strokeStyle = b.elite
      ? `rgba(120, 220, 255, ${0.05 + t * 0.32})`
      : `rgba(255, 80, 48, ${0.05 + t * 0.32})`;
    ctx.lineWidth = dh * (0.28 + t * 0.85);
    ctx.beginPath();
    ctx.moveTo(a.x, a.y);
    ctx.lineTo(c.x, c.y);
    ctx.stroke();
  ...
  if (img) {
    for (let i = 0; i < pts.length - 1; i++) {
      ...
      const sx = p.x - prev.x;
      const sy = p.y - prev.y;
      const ang = sx === 0 && sy === 0 ? Math.atan2(b.vy, b.vx) : Math.atan2(sy, sx);
      const s = 0.52 + t * 0.38;
      ctx.save();
      ctx.globalAlpha = 0.08 + t * 0.34;
      ctx.translate(p.x, p.y);
      ctx.rotate(ang);
      ctx.imageSmoothingEnabled = true;
      ctx.drawImage(img, -dw * s, -dh * s * 0.5, dw * s, dh * s);
      ctx.restore();
    }
  }
  ctx.restore();
